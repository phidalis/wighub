...
function checkLiveSupportQueue() {
    ...
    if (liveSupportTab && waitingRequests.length > 0) {
        let badge = liveSupportTab.querySelector('.notification-badge');
        if (!badge) {
            badge = document.createElement('span');
            badge.className = 'notification-badge';
            badge.style.cssText = 'background: #28a745; margin-left: 5px; padding: 2px 6px; border-radius: 10px; font-size: 11px;';
            liveSupportTab.appendChild(badge);
        }
        badge.textContent = waitingRequests.length;
        badge.style.display = 'inline';
    } else if (liveSupportTab) {
        const badge = liveSupportTab.querySelector('.notification-badge');
...
function renderLiveSupportTab() {
    ...
    let html = `
        <div style="margin-bottom: 20px;">
            <h3><i class="fas fa-clock"></i> Waiting for Support (${waiting.length})</h3>
    `;
    
    if (waiting.length === 0) {
        html += '<p class="empty-state">No waiting requests</p>';
    } else {
        html += '<div style="display: grid; gap: 15px;">';
        waiting.forEach(req => {
            const time = new Date(req.timestamp).toLocaleTimeString();
            html += `
                <div class="support-request" style="background: #fff3cd; padding: 15px; border-radius: 8px; border-left: 4px solid #ffc107;">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div style="flex:1;">
                            <strong>${req.username}</strong> (${req.userType})
                            <div style="font-size: 12px; color: #666;">${time}</div>
                            <div style="margin-top: 10px; background: white; padding: 10px; border-radius: 5px;">
                                "${req.question}"
                            </div>
                        </div>
                        <button class="view-btn" onclick="joinLiveChat('${req.id}')" style="background: #28a745; padding: 10px 20px; margin-left: 15px;">
    ...
    html += `
        ...
        <div style="margin-top: 30px;">
            <h3><i class="fas fa-comments"></i> Your Active Chats (${active.length})</h3>
    `;
    
    if (active.length === 0) {
        html += '<p class="empty-state">No active chats</p>';
    } else {
        html += '<div style="display: grid; gap: 15px;">';
        active.forEach(chat => {
            const time = new Date(chat.timestamp).toLocaleTimeString();
            html += `
                <div class="active-chat" style="background: #d4edda; padding: 15px; border-radius: 8px; border-left: 4px solid #28a745;">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <strong>${chat.username}</strong>
                            <div style="font-size: 12px;">Started: ${time}</div>
                            <div style="margin-top: 5px;">Last message: ${chat.lastMessage || 'No messages yet'}</div>
                        </div>
                        <button class="view-btn" onclick="openLiveChat('${chat.id}')" style="background: #3498db;">
...
function joinLiveChat(requestId) {
    ...
    supportQueue[requestIndex].messages = supportQueue[requestIndex].messages || [];
    
    // Add system message
    supportQueue[requestIndex].messages.push({
        from: 'system',
        message: `${adminUser.name} has joined the chat`,
