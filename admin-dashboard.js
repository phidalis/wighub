...
function loadCurrentAdmin() {
    ...
    if (adminUser) {
        try {
            ...
            // Add menu items immediately after admin is loaded
...
// ===== UPDATED ADD PRODUCT FUNCTION WITH SEQUENTIAL IDS =====
function addProduct() {
    // Get form values
    const productId = document.getElementById('productId')?.value.trim() || '';
    ...
    // Determine product ID - ENSURE SEQUENTIAL NUMBERS
    let newProductId;
    
    if (productId) {
        // Use manual ID (could be text like "WIG001")
        newProductId = productId;
    } else {
        // Generate sequential numeric ID starting from 1
        if (allProducts.length === 0) {
            newProductId = 1; // First product
        } else {
            ...
            const numericIds = allProducts
                .map(p => {
                    ...
                    if (typeof id === 'string' && /^\d+$/.test(id)) return parseInt(id);
                    return 0;
                })
                .filter(id => id > 0);
            
            if (numericIds.length > 0) {
                newProductId = Math.max(...numericIds) + 1;
            } else {
...
// ===== UPDATED LOAD SAMPLE PRODUCTS WITH SEQUENTIAL IDS =====
function loadSampleProducts() {
    const sampleProducts = [
        {
            id: 1, // Changed from "WIG001" to 1
    ...
    saveProducts();
    showAdminNotification('✅ Sample products loaded successfully! (IDs: 1-6)', 'success');
    showAdminSection('products');
}
...
// Load admin list with roles
function loadAdminList() {
    if (!hasPermission('employee.view')) {
        document.getElementById('adminList').innerHTML = '<p style="color: #e74c3c; padding: 20px;">You don\'t have permission to view employees</p>';
        return;
    }
    ...
    const currentUserEmail = currentAdmin ? currentAdmin.email : localStorage.getItem('adminUser');
    ...
    admins.forEach(admin => {
        ...
        html += `
            <tr style="border-bottom: 1px solid #e0e0e0;">
                ...
                <td style="padding: 12px;">
                    <span class="status-badge ${admin.status === 'active' ? 'active' : 'suspended'}" 
                          style="background: ${admin.status === 'active' ? '#4CAF50' : '#f44336'}; color: white;">
                        ${admin.status || 'active'}
                    </span>
                </td>
        ...
        // Edit button - only super_admin can edit others
        if (hasPermission('employee.edit') && (currentAdmin.role === 'super_admin' || admin.id === currentAdmin.id)) {
            html += `<button class="action-btn edit" onclick="editAdmin(${admin.id})" title="Edit Admin" style="background: #3498db; color: white; border: none; padding: 6px 10px; border-radius: 4px; cursor: pointer;">
                        <i class="fas fa-edit"></i>
                    </button>`;
        }
        
        // Delete button - only super_admin and not self
        if (hasPermission('employee.delete') && currentAdmin.role === 'super_admin' && admin.email !== currentUserEmail) {
            html += `<button class="action-btn delete" onclick="deleteAdmin(${admin.id})" title="Delete Admin" style="background: #e74c3c; color: white; border: none; padding: 6px 10px; border-radius: 4px; cursor: pointer;">
                        <i class="fas fa-trash"></i>
                    </button>`;
        }
        
        // Suspend/Activate button - only super_admin
        if (hasPermission('employee.edit') && currentAdmin.role === 'super_admin' && admin.email !== currentUserEmail) {
            ...
            html += `<button class="action-btn" onclick="toggleAdminStatus(${admin.id})" title="${action} Admin" style="background: ${bgColor}; color: white; border: none; padding: 6px 10px; border-radius: 4px; cursor: pointer;">
...
function toggleAdminStatus(adminId) {
    ...
    if (index !== -1) {
        const newStatus = admins[index].status === 'active' ? 'suspended' : 'active';
        const action = newStatus === 'active' ? 'activate' : 'suspend';
        
        if (confirm(`Are you sure you want to ${action} ${admins[index].name}?`)) {
            admins[index].status = newStatus;
            admins[index].statusChangedAt = new Date().toISOString();
            admins[index].statusChangedBy = currentAdmin.email;
            
            localStorage.setItem('wigAdmins', JSON.stringify(admins));
            
            logAdminAction('EMPLOYEE_STATUS_CHANGE', {
                employeeId: adminId,
                employeeName: admins[index].name,
                oldStatus: admins[index].status === 'active' ? 'suspended' : 'active',
                newStatus: newStatus
            });
            
            showAdminNotification(`✅ ${admins[index].name} ${action}d successfully`, 'success');
            loadAdminList();
        }
    }
}

// Show add admin form (with role selection)
...
function addNewAdmin() {
    ...
    logAdminAction('EMPLOYEE_CREATE', {
        ...
        employeeName: newAdmin.name,
        employeeEmail: newAdmin.email,
        employeeRole: newAdmin.role
    });
...
function editAdmin(adminId) {
    ...
    if (currentAdmin.role === 'super_admin') {
        ...
        newRole = prompt('Enter new role (super_admin/admin/manager/support/finance/content):', admin.role);
    ...
    admin.updatedBy = currentAdmin.email;
