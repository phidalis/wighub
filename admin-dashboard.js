...
// In loadCurrentAdmin function
function loadCurrentAdmin() {
    const adminUser = localStorage.getItem('adminUser');
    if (adminUser) {
        try {
            currentAdmin = JSON.parse(adminUser);
            console.log('✅ Current admin loaded:', currentAdmin.email, 'Role:', currentAdmin.role);
            
            // Add role if missing
            if (!currentAdmin.role) {
                currentAdmin.role = 'super_admin';
                console.log('⚠️ Role missing, set to super_admin');
            }
            ...
            if (adminInfo) {
                ...
                switch(currentAdmin.role) {
                    case 'super_admin': roleDisplay = '👑 Owner'; break;
                    case 'admin': roleDisplay = '⚙️ Admin'; break;
                    case 'manager': roleDisplay = '📋 Manager'; break;
                    case 'support': roleDisplay = '🎧 Support'; break;
                    case 'finance': roleDisplay = '💰 Finance'; break;
                    case 'content': roleDisplay = '📝 Content'; break;
                    default: roleDisplay = '👤 Staff';
                }
                adminInfo.innerHTML = `${currentAdmin.name} <span style="font-size: 11px; background: rgba(255,255,255,0.2); padding: 2px 8px; border-radius: 12px; margin-left: 5px;">${roleDisplay}</span>`;
            }
...
async function loadProducts() {
    ...
    try {
        ...
        console.log('✅ Loaded products from Firestore:', allProducts.length);
...
function loadProductsFromLocalStorage() {
    ...
    if (productsJSON) {
        try {
            ...
            filteredProducts = [...allProducts];
            console.log('Loaded from localStorage:', allProducts.length);
        } catch (e) {
        ...
        // Auto-load sample products if empty
        if (confirm('No products found. Would you like to load sample products?')) {
            loadSampleProducts();
        }
    }
            ...
            <tr>
                ...
                <td>
                    <div style="display: flex; gap: 5px;">
                        <button class="action-btn edit" onclick="openEditPopup('${product.id}')" 
...
/**
 * Update default admin (if using localStorage fallback)
 */
function updateDefaultAdmin() {
    ...
    if (defaultAdminIndex !== -1) {
        ...
        alert('Default admin not found in localStorage');
    }
}
...
// ===== AUDIT LOG FUNCTIONS - FIXED VERSION =====
