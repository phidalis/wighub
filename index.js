...
let unsubscribeHeroImages = null; 
let heroImages = []; // Will be populated from Firestore
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🏠 WigHub Home Page - Fast Loading Mode');
    ...
    // Start loading products IMMEDIATELY (no delay)
    loadFeaturedProducts();
    
    setupEventListeners();
    setupScrollToTop();
    initCartBadge();
    // Initialize hero carousel (loads once from cache/database)
    initHeroCarousel();
  
    // Setup real-time listener after initial load (doesn't block)
    setTimeout(() => {
        if (typeof setupRealTimeProducts === 'function') {
...
window.retryLoadProducts = function() {
    console.log('🔄 Manual retry loading products...');
    showLoading();
    loadFeaturedProducts();
};

// Load featured products from Firebase Firestore with timeout and cache
async function loadFeaturedProducts() {
    ...
    // Set a timeout fallback (8 seconds max)
    ...
    try {
        ...
        localStorage.setItem('wigProductsCache', JSON.stringify(allProducts));
        localStorage.setItem('wigProductsCacheTime', Date.now());
        ...
        const cachedProducts = localStorage.getItem('wigProductsCache');
        if (cachedProducts) {
            const cacheTime = localStorage.getItem('wigProductsCacheTime');
            ...
            if (cacheAge < 3600000) {
                ...
                filterHomeProducts();
                showNotification('Using cached products (check your connection)', 'info');
                return allProducts;
            }
        }
    ...
    if (productsContainer) {
        productsContainer.innerHTML = `
            <div class="error-state" style="text-align: center; padding: 80px 20px;">
                <i class="fas fa-wifi" style="font-size: 64px; color: #dc3545; margin-bottom: 20px; opacity: 0.7;"></i>
                <h3 style="margin-bottom: 12px; color: #dc3545;">Connection Issue</h3>
                <p style="color: #666; max-width: 300px; margin: 0 auto 20px;">${message}</p>
                <button onclick="retryLoadProducts()" style="background: #8A2BE2; color: white; border: none; padding: 12px 28px; border-radius: 30px; font-size: 14px; font-weight: 600; cursor: pointer; box-shadow: 0 2px 8px rgba(138,43,226,0.3);">
                    <i class="fas fa-sync-alt"></i> Retry Now
                </button>
                <p style="margin-top: 15px; font-size: 12px; color: #999;">
                    <i class="fas fa-info-circle"></i> Check your internet and try again
                </p>
            </div>
        `;
    }
...
// Show loading indicator - INSTANT VISUAL FEEDBACK
function showLoading() {
    const productsContainer = document.getElementById('featuredProducts');
    if (productsContainer) {
        productsContainer.innerHTML = `
            <div class="loading-state" style="text-align: center; padding: 80px 20px;">
                <div class="spinner" style="width: 60px; height: 60px; border: 4px solid #f3f3f3; border-top: 4px solid #8A2BE2; border-radius: 50%; margin: 0 auto 20px; animation: spin 0.8s linear infinite;"></div>
                <p style="color: #666; font-size: 16px;"><i class="fas fa-spinner fa-pulse"></i> Loading products...</p>
                <p style="color: #999; font-size: 13px; margin-top: 8px;">Please wait, fetching from our store</p>
            </div>
        `;
    }
}
...
// ===== HERO CAROUSEL FUNCTIONS - OPTIMIZED VERSION =====
let currentSlide = 0;
let slideInterval;
let heroImagesLoaded = false;

// Load hero images from Firestore (ONCE) with caching
async function loadHeroImages() {
    ...
    if (heroImagesLoaded && window.heroImages && window.heroImages.length > 0) {
    ...
    try {
        ...
        const cached = localStorage.getItem('wigHeroImagesCache');
        const cacheTime = localStorage.getItem('wigHeroImagesCacheTime');
        
        if (cached && cacheTime && (Date.now() - parseInt(cacheTime) < 3600000)) {
            try {
                window.heroImages = JSON.parse(cached);
                if (window.heroImages && window.heroImages.length > 0) {
                    console.log('📦 Loaded hero images from cache instantly');
                    createCarouselSlides();
                    startSlideShow();
                }
            } catch(e) {
        ...
        // Fetch fresh data from Firestore (only once)
        console.log('🔄 Fetching fresh hero images from Firestore...');
        const heroRef = collection(window.db, 'heroImages');
        ...
        querySnapshot.forEach(doc => {
            ...
            if (data.status === 'active') {
                newHeroImages.push({
                    ...
                    title: data.title || 'Find Your Perfect Style',
                    subtitle: data.subtitle || 'Premium quality products',
                    order: data.order || 0,
                    status: data.status
                });
            }
        ...
        // Sort by order
        newHeroImages.sort((a, b) => (a.order || 0) - (b.order || 0));
        
        // Only update if we have images
        if (newHeroImages.length > 0) {
            window.heroImages = newHeroImages;
            
            // Update cache
            localStorage.setItem('wigHeroImagesCache', JSON.stringify(newHeroImages));
            localStorage.setItem('wigHeroImagesCacheTime', Date.now());
            
            // Re-create carousel if needed (or update existing)
            createCarouselSlides();
            startSlideShow();
            console.log('✅ Hero images loaded from Firestore:', newHeroImages.length);
        } else if (!window.heroImages || window.heroImages.length === 0) {
            showEmptyHeroState();
        }
        ...
        // If cache exists but fetch failed, keep using cache
        if (!window.heroImages || window.heroImages.length === 0) {
            showEmptyHeroState();
        }
    }
...
// Initialize carousel (called once on page load)
    ...
    if (cached) {
        try {
            ...
            if (window.heroImages && window.heroImages.length > 0) {
                createCarouselSlides();
                startSlideShow();
                console.log('📦 Initial carousel from cache');
            }
        } catch(e) {}
    }
    
    // Load fresh data in background (only once)
    await loadHeroImages();
}

// Show empty state when no hero images are available
function showEmptyHeroState() {
    const slidesContainer = document.getElementById('heroSlides');
    if (slidesContainer) {
        slidesContainer.innerHTML = `
            <div class="carousel-slide active" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
                <div class="slide-content">
                    <h1>Welcome to Wigs and Beauty</h1>
                    <p>Premium quality wigs and beauty products</p>
...
function createCarouselSlides() {
    ...
    // If no hero images, show empty state
    if (!window.heroImages || window.heroImages.length === 0) {
        showEmptyHeroState();
        return;
    }
    ...
    window.heroImages.forEach((img, index) => {
        const imageUrl = img.imageUrl || img.image;
        slidesHtml += `
            <div class="carousel-slide ${index === 0 ? 'active' : ''}" style="background-image: url('${imageUrl}')">
                <div class="slide-content">
                    <h1>${escapeHtml(img.title || 'Find Your Perfect Style')}</h1>
                    <p>${escapeHtml(img.subtitle || 'Premium quality products')}</p>
                    <a href="client-login.html?tab=signup" class="cta-button">Start Shopping</a>
                </div>
            </div>
        `;
...
function startSlideShow() {
    ...
    if (window.heroImages && window.heroImages.length > 1) {
        slideInterval = setInterval(() => {
            changeSlide(1);
        }, 5000); // Change every 5 seconds (slower for better UX)
    }
}
