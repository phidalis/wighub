let unsubscribeBraiders = null;

function setupRealTimeBraiders() {
    if (!window.db) return;
    
    const braidersRef = collection(window.db, 'braiders');
    
    if (unsubscribeBraiders) unsubscribeBraiders();
    
    unsubscribeBraiders = onSnapshot(braidersRef, (snapshot) => {
        console.log('🔄 Braiders updated');
        
        const braiders = [];
        snapshot.forEach(doc => {
            braiders.push({ id: doc.id, ...doc.data() });
        });
        
        localStorage.setItem('braiders', JSON.stringify(braiders));
        
        if (typeof loadBraiders === 'function') loadBraiders();
        if (typeof loadAllBraiders === 'function') loadAllBraiders();
        
        showUpdateNotification('Braiders list updated!');
    });
}
...
function setupRealTimeHeroImages() {
    ...
    if (unsubscribeHeroImages) unsubscribeHeroImages();
    
    unsubscribeHeroImages = onSnapshot(heroRef, (snapshot) => {
        console.log('🔄 Hero images updated');
        
        const heroImages = [];
        snapshot.forEach(doc => {
            heroImages.push({ id: doc.id, ...doc.data() });
        });
        ...
        showUpdateNotification('Hero images updated!');
    });
}
// Global variables
let currentEditingId = null;

// Add permission check for braiders management
function checkBraiderAdminPermission() {
    const adminUser = localStorage.getItem('adminUser');
    if (!adminUser) return false;
    
    try {
        const currentAdmin = JSON.parse(adminUser);
...
document.addEventListener('DOMContentLoaded', function() {
    if (!checkBraiderAdminPermission()) {
        alert('⛔ You do not have permission to access the braiders admin panel.');
